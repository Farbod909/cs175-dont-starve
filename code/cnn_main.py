...
def transform_farm(farm_array):
    ...
    farm_tensor = torch.tensor(farm_array,dtype=torch.float).view(side_len,side_len) #converts array into square matrix
    farm_tensor = farm_tensor.unsqueeze_(0) #adds an extra dimension at the index specified. 
    return farm_tensor
